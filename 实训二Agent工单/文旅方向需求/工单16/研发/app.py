# -*- coding: utf-8 -*-
"""
work_order_16_文旅需求分析 - Web 应用入口
文旅智能体 - 需求分析与功能设计
工单编号：人工智能NLP-Agent数字人项目-16-文旅需求分析
"""
from fastapi import FastAPI, Request
from fastapi.responses import HTMLResponse, JSONResponse
from tourism_analyzer import analyzer
from config import WEB_HOST, WEB_PORT

app = FastAPI(title="工单16-文旅需求分析", version="1.0")
...
<body>
    <div class="header">
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-3">
                <i class="bi bi-buildings" style="font-size:1.8rem;color:var(--accent)"></i>
                <h1>文旅需求分析 Agent</h1>
            </div>
            <span class="badge">工单 16 · 文旅智能体</span>
        </div>
    </div>
        ...
        <div class="row g-4">
            <div class="col-lg-4">
                <div class="card-glass">
                    ...
                    <div class="mb-3">
                        <label class="text-muted mb-1" style="font-size:0.85rem">景区类型</label>
                        <select class="input-dark" id="scenicType">
                            <option value="历史文化景区">历史文化景区</option>
                            <option value="自然风光景区">自然风光景区</option>
                            <option value="主题乐园">主题乐园</option>
                            <option value="古镇古村">古镇古村</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="text-muted mb-1" style="font-size:0.85rem">重点需求（可选）</label>
                        <textarea class="input-dark" id="requirements" rows="3" placeholder="例如：提升游客导览体验，增加文化互动..."></textarea>
                    </div>
            ...
            <div class="col-lg-8">
                <div class="card-glass">
                    ...
                    <div class="result-area" id="resultArea"><span class="text-muted">选择景区类型后点击「开始需求分析」</span></div>
                </div>
            </div>
        </div>
    </div>
    <script>
        async function analyze() {
            ...
            const scenicType = document.getElementById('scenicType').value;
            ...
            try {
                const res = await fetch('/analyze', {
                    ...
                    body: JSON.stringify({ scenic_type: scenicType, requirements })
                });
        ...
        function formatResult(data) {
            ...
            // 2. AI 应用场景
            html += '<div class="scenario-card"><h6><i class="bi bi-robot"></i> AI 数字人应用场景</h6>';
...
async def analyze_endpoint(request: Request):
    ...
    try:
        ...
        scenic_type = data.get("scenic_type", "历史文化景区")
        requirements = data.get("requirements", "")
        
        # 1. 痛点分析
        pain_points = analyzer.analyze_pain_points(scenic_type)
...
@app.get("/health")
async def health():
    return {"status": "ok", "work_order": "work_order_16_文旅需求分析"}


if __name__ == "__main__":
    import uvicorn
    print(f"\n{'='*60}")
    print(f"🏛️  工单 16 - 文旅需求分析")
    print(f"{'='*60}")
